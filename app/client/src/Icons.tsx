...
const Icons = ({ name, color, size = 24, isAddon }: IconsProps) => {
	return (
		<>
			{/* UI */}
			{name === "arrow" && (
				<svg {...baseProps(24, size, color, isAddon)}>
			...
			{name === "league-of-legends" && (
				<svg {...baseProps(96, size, color, isAddon)} strokeWidth={4}>
			...
			{name === "modify" && (
				<svg {...baseProps(48, size, color, isAddon)}>
			...
			{name === "screenshot" && (
				<svg {...baseProps(14, size, color, isAddon)}>
					<g
						fill="none"
						stroke="currentColor"
						stroke-linecap="round"
